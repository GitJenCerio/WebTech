...
>(({ className, children, viewportClassName, ...props }, ref) => (
  <DialogPortal>
    <div
      className={cn(
        "fixed inset-0 z-50 flex items-end sm:items-center justify-center overflow-y-auto overscroll-contain px-2 sm:px-3 pt-2",
    ...
    >
      ...
      >
        {children}
        <DialogPrimitive.Close className="absolute right-2 top-2 z-20 rounded-none p-1.5 text-muted-foreground opacity-70 ring-offset-pearl transition-all hover:opacity-100 hover:bg-ash hover:text-ink focus:outline-none focus:ring-2 focus:ring-champagne focus:ring-offset-2 disabled:pointer-events-none">
          <X className="h-4 w-4" />
...
const DialogFooter = ({
  ...
  <div
    className={cn(
      "grid grid-cols-2 sm:grid-cols-3 gap-2 pt-2 mt-auto shrink-0 border-t border-border/80 bg-pearl z-10 sm:justify-end",
      "pb-[max(0.25rem,env(safe-area-inset-bottom,0px))]",
      "[&>*]:min-h-10 [&>*]:text-xs [&>*]:w-full sm:[&>*]:min-h-10 sm:[&>*]:text-sm",
      className
    )}
    {...props}
  />
